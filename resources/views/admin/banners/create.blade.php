...
@section('content')
    <div class="max-w-lg bg-white rounded-2xl border border-warm-gray/50 shadow-sm p-8">
        <form action="{{ route('admin.banners.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
            ...
            <div>
                <label class="block text-sm font-medium text-forest-700 mb-2">Subtitle</label>
                <input type="text" name="subtitle" value="{{ old('subtitle') }}" class="w-full px-4 py-3 rounded-xl border border-warm-gray bg-cream text-forest-800 focus:ring-2 focus:ring-forest-500 focus:border-transparent outline-none">
            ...
            <div>
                <label class="block text-sm font-medium text-forest-700 mb-2">Link URL</label>
                <input type="url" name="link" value="{{ old('link') }}" class="w-full px-4 py-3 rounded-xl border border-warm-gray bg-cream text-forest-800 focus:ring-2 focus:ring-forest-500 focus:border-transparent outline-none">
                @error('link') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-forest-700 mb-2">Page</label>
                <select name="page" class="w-full px-4 py-3 rounded-xl border border-warm-gray bg-cream text-forest-800 focus:ring-2 focus:ring-forest-500 focus:border-transparent outline-none">
                    <option value="">Select Page</option>
                    <option value="home" {{ old('page') == 'home' ? 'selected' : '' }}>Home</option>
                    <option value="catalog" {{ old('page') == 'catalog' ? 'selected' : '' }}>Catalog</option>
                    <option value="about" {{ old('page') == 'about' ? 'selected' : '' }}>About</option>
                    <option value="contact" {{ old('page') == 'contact' ? 'selected' : '' }}>Contact</option>
                </select>
                @error('page') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
            ...
            <div class="flex items-center gap-4">
                <button type="submit" class="px-6 py-3 bg-forest-700 text-white font-semibold rounded-xl hover:bg-forest-600 transition-all">Create Banner</button>
                <a href="{{ route('admin.banners.index') }}" class="px-6 py-3 bg-white text-forest-700 font-semibold rounded-xl border border-warm-gray/50 hover:bg-forest-50 transition-all">Cancel</a>
            </div>
        </form>
    </div>
@endsection
